...
function quicklabels($nums, $title_p = "0", $accessToken) {
    ...
    if ($title_p == '1') {
        ...
        //$url = URL . '/?q=oclc:1033443432'; // oof serait complique de chercher le barcode correspondant
		#var_dump($url);
        ...
        try {
			...
			// //// PAS BESOIN !! get bib, my add
			// // $url2 = URL . '/?q=oclc:' . $oclc;
        	// // $response2 = $client->request('GET', $url2, ['headers' => $headers]);
        	// // $xml2 = new SimpleXMLElement($response2->getBody());			
			// $holdings =  $xml->entry->content->copy->holding; // plus bas, il loop sur les holdings
			// foreach ($holdings as $holding) {
			// 	if ($barcode == $holding->pieceDesignation) {
			// 		// $hold = $holding;
			// 		$enum = $holding->caption->enumeration[0];
			// 		if (!is_null($enum)){
			// 			$enumText = (string)$enum->attributes()[0] . ' ' . $enum;
			// 		}
			// 	}
			// };
        	...
        	if ($rectype == "SERIAL") {
        		...
        		$enum = $copy->holding[$j]->caption->enumeration[0];
        		if ($enum) {
					$enumText = (string)$enum->attributes()[0] . ' ' . $enum;
					$callnum = $callnum . " " . $enumText;
        		}
        	...
        	$c = strcspn($location, ":");
        	$location = substr($location, 0, $c); // my comment : = shelving location
			...
			// my add
        	$return_call_number = str_replace("V<br />.<br />", "V. ", $return_call_number);
        	
			// Return array of call number, pocket label text
			// my add (var_dump)
			// var_dump(array("$return_call_number", "$return_title<br />$return_author<br />$print_call_num"));
